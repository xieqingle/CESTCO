...
object DrawableHelper{
    ...
    fun createBitmapFromView(view: View, scale: Float): Bitmap? {
        if (view is ImageView) {
            val drawable = (view as ImageView).getDrawable()
            if (drawable != null && drawable is BitmapDrawable) {
                return (drawable as BitmapDrawable).bitmap
            }
        }
        view.clearFocus()
        val bitmap = createBitmapSafely((view.getWidth() * scale) as Int,
                (view.getHeight() * scale) as Int, Bitmap.Config.ARGB_8888, 1)
        if (bitmap != null) {
    ...
    fun createBitmapFromView(view: View, leftCrop: Int, topCrop: Int, rightCrop: Int, bottomCrop: Int): Bitmap? {
        val originBitmap = createBitmapFromView(view)
                ?: return null
        val cutBitmap = createBitmapSafely(view.getWidth() - rightCrop - leftCrop, view.getHeight() - topCrop - bottomCrop, Bitmap.Config.ARGB_8888, 1)
                ?: return null
        val canvas = Canvas(cutBitmap)
        val src = Rect(leftCrop, topCrop, view.getWidth() - rightCrop, view.getHeight() - bottomCrop)
        val dest = Rect(0, 0, view.getWidth() - rightCrop - leftCrop, view.getHeight() - topCrop - bottomCrop)
        canvas.drawColor(Color.WHITE) // 防止 View 上面有些区域空白导致最终 Bitmap 上有些区域变黑
...
//            QMUILog.d(TAG, "Error in getVectorDrawable. resVector=" + resVector + ", resName=" + context.getResources().getResourceName(resVector) + e.message)
            return null
        }
